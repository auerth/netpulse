...
import netpulseLogo from './assets/netpulse_logo.png';
const App = () => {
  // --- BRAND COLORS & FONTS ---
  const colors = {
    bg: '#FFFFFF',
  ...
  const [speedInterval, setSpeedInterval] = useState(1);
    ...
    <div className="min-h-screen bg-white p-4 md:p-8 relative" style={{ fontFamily: "'Montserrat', sans-serif", color: colors.text }}>
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white  shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col">
            ...
            <div className="p-6 space-y-6">
              ...
              {selectedLog.history && selectedLog.history.length > 0 ? (
                <div className="h-64 w-full border border-slate-100  p-4 bg-white shadow-sm">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* HEADER BRANDING */}
        <header className="grid grid-cols-1 lg:grid-cols-4 gap-8 full-w">
       
            <div className=" flex items-center justify-center header-image" >
              {/* 2. Die importierte Variable nutzen (in geschweiften Klammern): */}
              <img src={netpulseLogo} alt="NetPulse Logo" className="w-full h-full object-contain" />
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-6  shadow-lg border border-slate-100">
              ...
              <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {downloadPresets.map(preset => (
                            <button
                              ...
                              disabled={isRunning}
                              className={`text-[10px] font-bold py-1.5 rounded border transition ${downloadUrl === preset.url
                ...
                <div className="pt-4">
                  <button
                    onClick={toggleStart}
                    className={`w-full flex items-center justify-center gap-3 py-4  font-bold text-white shadow-lg transition-transform active:scale-95 ${isRunning ? 'hover:bg-red-600' : 'hover:opacity-90'}`}
                ...
                {logs.length > 0 && !isRunning && (
                  <button onClick={downloadCSV} className="w-full flex items-center justify-center gap-2 py-3  border-2 border-slate-100 text-slate-500 font-medium hover:bg-slate-50 hover:text-slate-700 transition text-sm">
            ...
            {downloadState.active && (
              <div className="bg-white p-5  shadow-lg border border-blue-100 animate-in slide-in-from-left-2 duration-300">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white p-5  shadow-md border border-slate-100 col-span-2">
...
const InputGroup = ({ label, value, setValue, disabled }) => (
  <div>
    ...
    <input
      ...
      disabled={disabled}
      min="1"
      className="w-full bg-slate-50 border border-slate-200  px-4 py-3 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition disabled:opacity-50"
    />
  </div>
);

const KpiTile = ({ label, value, unit, color, isAlert }) => (
  <div className={`p-5  border shadow-sm flex flex-col justify-between h-32 ${isAlert ? 'bg-red-50 border-red-100' : 'bg-white border-slate-100'}`}>
...
const ChartContainer = ({ title, subTitle, icon, color, children }) => (
  <div className="bg-white p-6  shadow-lg border border-slate-100 h-80 relative overflow-hidden group hover:shadow-xl transition-shadow duration-300">
